    ...
    try {
        ...
        for (const m of items.slice().reverse()) {
            ...
            if (m.msg_type === 'text') {
                ...
                const matched = rawContent.match(/\[liangge-cmd\] approved=true release_id=(\S+) project=(\S+) version=(\S+) tarball_url=(\S+) sha256=([0-9a-fA-F]{64})/);
                if (!matched) {
                    continue; // 不 markProcessed, 等下次 poll 再看是否 patch 上了 meta
                }
                cmd = {
                    msg_type: 'release_command',
                    release_id: matched[1],
                    project: matched[2],
                    version: matched[3],
            ...
            console.log(kleur.cyan('▶'), `收到指令 (poll/${m.msg_type}): ${cmd.project} ${cmd.version} release_id=${cmd.release_id}`);
        ...
        console.error('[poller] 异常:', err.message);
    }
}
export function startPoller(cfg) {
    _startTimeSec = Math.floor(Date.now() / 1000);
    console.log(kleur.green('✓'), `飞书消息 poller 已启动 (每 ${POLL_INTERVAL_MS / 1000}s 拉一次, 启动前消息忽略)`);
    setInterval(() => {
